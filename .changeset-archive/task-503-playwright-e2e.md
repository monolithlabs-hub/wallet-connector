...
**Wiring:** root `playwright.config.ts` + `e2e/{accessibility,desktop-flow,mobile-flow}.spec.ts`. Root `package.json` adds `@playwright/test@^1.60.0` as a devDep plus two scripts: `pnpm e2e` (run the suite) and `pnpm e2e:install` (install the browser binaries). `examples/react-example/` is a standalone workspace package — Vite 8 + React 19 + `@vitejs/plugin-react@6` + workspace deps on `@monolithlabs-hub/wallet-connect-{core,react}`. The `.gitignore` is extended to drop Playwright's per-run artifacts (`test-results/`, `playwright-report/`).

...
| `icon`           | `string`     | yes      | URL or `data:` URI for the wallet's logo. An empty string renders a small placeholder box; provide a real icon for a polished modal.                                                                                                                                   |
| `deepLinkScheme` | `string`     | no       | Custom URL scheme (e.g., `phantom://`) used by the mobile deep-link probe.                                                                                                                                                                                             |
| `universalLink`  | `string`     | no       | HTTPS universal link (e.g., `https://opindex.deeptap.io`). **Its presence is what makes a wallet deep-linkable on mobile** — omit it to mark a wallet install/open-only (the modal routes taps to `installUrl` instead).                                               |
| `appStoreUrl`    | `string`     | no       | iOS App Store URL. Used as fallback when the deep link isn't intercepted (i.e., wallet not installed).                                                                                                                                                                 |
| `playStoreUrl`   | `string`     | no       | Google Play URL. Android fallback.                                                                                                                                                                                                                                     |
| `installUrl`     | `string`     | no       | Mobile download / landing page (e.g. `https://opindex.deeptap.io`). The primary not-installed fallback target on mobile — takes precedence over the store URLs.                                                                                                        |
| `extensionUrl`   | `string`     | no       | Desktop browser-extension install page (e.g. a Chrome Web Store listing). On the desktop `install-prompt` strategy, selecting the wallet opens this in a new tab (falling back to `installUrl`).                                                                       |
| `standardName`   | `WalletName` | no       | The wallet's Wallet Standard registration name. The manager pairs the configured wallet to its `StandardWalletAdapter` by this name first; if omitted, it falls back to case-insensitive matching on `name`. Use `asWalletName('Opindex Wallet')` to brand the string. |

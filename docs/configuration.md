...
| `standardName`   | `WalletName` | no       | The wallet's Wallet Standard registration name. The manager pairs the configured wallet to its `StandardWalletAdapter` by this name first; if omitted, it falls back to case-insensitive matching on `name`. Use `asWalletName('Phantom')` to brand the string. |

...
TASK-105 — add the DeepLinkBuilder to `@monolithlabs/wallet-connect-core`. Exports `buildConnectUrl(wallet, options)`, `buildSignAndConnectUrl(wallet, options)`, `generateEphemeralKeypair()`, plus the `EphemeralKeypair`, `ConnectOptions`, `SignConnectOptions`, and `SolanaCluster` types.
